...
BOOL CMyButton2::Attach(const UINT nID, CWnd* pParent)
{
...
void CMyButton2::SetDownColor(COLORREF color)
{
	m_DownColor = color;
}
void CMyButton2::SetUpColor(COLORREF color)
{
	m_UpColor = color;
}
...
BOOL CMyButton2::OnWndMsg(UINT message, WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	// 捕获鼠标左键按下和松开状态，上报父窗口
	switch(message)
	{
	case WM_LBUTTONDOWN:
		OUTPUT_LOG("downC");
		::PostMessage(m_hande2,WM_DOWN_C,0,0);
		//m_hWnd
		break;
	case WM_LBUTTONUP:
		OUTPUT_LOG("upC");
		::PostMessage(m_hande2,WM_UP_C,0,0);
		break;
	default:
		break;
	}
...
void CMyButton2::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (!_bMouseTrack)
	{
		...
		::PostMessage(m_hande2,WM_BUTTON_E,0,0);
	}
...
void CMyButton2::OnMouseLeave()
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	_bMouseTrack = FALSE;
	::PostMessage(m_hande2,WM_BUTTON_F,0,0);
